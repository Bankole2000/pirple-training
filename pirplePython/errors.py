"""
File: errors.py V1.0 (vehicles.py V2.0)
Auth: Bankole Esan
Desc: Pirple.com Python Homework #11
Task: Error Handling - Bullet Proof all inputs
"""
    ...
    if len(player.cars) > 0:
        if player.isDriving == False:
            ...
            try:
                ...
                print("Caught an error - Please be sure to enter a number")
                selectedCarIndex = -1
            while selectedCarIndex not in range(1, len(player.cars) + 1):
                print("Sorry, I don't think you have that car in your garage")
                try:
                    selectedCarIndex = int(
                        input(f"\nPlease Select A Car (1 - {len(player.cars)}): "))
                except:
                    print("Caught an error - Please enter a valid number")
                    selectedCarIndex = -1
...
def selectPlane(player):
    if len(player.planes) > 0:
        if player.isFlying == False:
            ...
            while selectedPlaneIndex not in range(1, len(player.planes) + 1):
...
def createNewCar(player):
    ...
    while True:
        try:
            year = int(input("(3/4) What year was the car made? "))
        except:
            print("Caught an error - Please enter a valid number")
            continue
        else:
    ...
    while True:
        try:
            weight = int(
                input("(4/4) What's the weight of the car (in pounds)? "))
        except:
            print("Caught an error - Please enter a valid number")
            continue
        else:
...
def createNewPlane(player):
    ...
    while True:
        try:
            year = int(input("(3/4) What year was the plane made? "))
        except:
            print("Caught an error - Please enter a valid number")
            continue
        else:
    ...
    while True:
        try:
            weight = int(
                input("(4/4) What's the weight of the plane (in pounds)? "))
        except:
            print("Caught an error - Please enter a valid number")
            continue
        else:
...
def repairVehicle(player):
    ...
    while True:
        try:
            ...
            if repairOption not in range(1, i + 1):
                print("Caught error - selection not in range")
                continue
            else:
                repairOption = int(repairOption)
                break
    if repairOption == i:
        if len(player.cars) > 0:
            for car in player.cars:
...
print("Release Notes: This game has been updated to handle all user input errors\n")
